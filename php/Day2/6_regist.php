...
<body>
    <h2>회원가입</h2>
    <form name="regform" id="regform" method="post" action="6_regist_ok.php" onsubmit="return sendit()">       <!--onsubmit 서브밋이 발생하게되면!-->
        <input type="hidden" name="isssn" id="isssn" value="n">  <!--눈에 보이지 않는태그, 검증버튼누르면 value y로 바꿔서 넘어가게하기위해 -->
        ...
        <p>주민등록번호 : <input type="text" name="ssn1" id="ssn1" maxlength="6" onkeyup="moveFocus()">
             -<input type="password" name="ssn2" id="ssn2" maxlength="7"> <input type="button" value="주민등록번호 검증" onclick="ssnCheck()"
        ></p>
        <p><label>우편번호 : <input type="text" name="zipcode" id="sample6_postcode" readonly></label> <input type="button" onclick="sample6_execDaumPostcode()" value="우편번호 검색"></p>
        ...
        <!--type submit으로 만들면 넘어가니까 button으로 바꿔줌  근데 그러면 다 넘어가니까 js에서 submit을 넣어줌
        이거.... 한꺼번에 할 수 있는 방법은 없을까 ????-->
